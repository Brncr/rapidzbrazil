...
const PitchDeckPage: React.FC = () => {
    ...
    const SlideCreators: React.FC = () => (
        <div className="flex flex-col justify-center h-full px-4 sm:px-8 md:px-20 overflow-y-auto py-16">
            <p className="text-xs sm:text-sm font-bold text-yellow-400 tracking-widest uppercase mb-3 sm:mb-4">03 — Confirmed Creators</p>
            <h2 className="text-xl sm:text-3xl md:text-4xl font-black text-white mb-2">
                <span className="text-yellow-400">{creators.length}</span> creators confirmed
            </h2>
            <p className="text-xs sm:text-sm text-white/40 mb-6 sm:mb-8">Hand-picked crypto influencers ready to produce content</p>
            {creators.length === 0 ? (
                <p className="text-white/30 text-sm">Loading creators...</p>
            ) : (
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-5 max-w-3xl">
                    {creators.map((c) => (
                        <a
                            key={c.name}
                            ...
                            className="bg-white/5 border border-white/10 rounded-xl p-3 sm:p-5 text-center hover:border-yellow-500/30 hover:bg-white/[0.08] transition block"
                        >
                            {c.image ? (
                                <img src={c.image} alt={c.name} className="w-10 h-10 sm:w-16 sm:h-16 rounded-full mx-auto mb-2 sm:mb-3 object-cover border-2 border-yellow-500/30" />
                            ) : (
                                <div className="w-10 h-10 sm:w-16 sm:h-16 rounded-full mx-auto mb-2 sm:mb-3 bg-yellow-500/20 flex items-center justify-center border-2 border-yellow-500/30">
                                    <span className="text-sm sm:text-xl font-black text-yellow-400">{c.name?.[0]}</span>
                                </div>
                            )}
                            <p className="text-xs sm:text-sm font-bold text-white truncate block">{c.name}</p>
                            {c.handle && <p className="text-[9px] sm:text-[10px] text-white/40 truncate block mt-0.5">{c.handle}</p>}
                            <div className="flex items-center justify-center gap-1 sm:gap-2 mt-2 sm:mt-3 flex-wrap">
                                {c.followers && (
                                    <span className="text-[8px] sm:text-[9px] px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-full bg-blue-500/15 text-blue-400 font-bold">
                                        👥 {c.followers}
                                    </span>
                                )}
                                {c.category && (
                                    <span className="text-[8px] sm:text-[9px] px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-full bg-purple-500/15 text-purple-400 font-bold">
                                        {c.category}
                                    </span>
                                )}
                            </div>
